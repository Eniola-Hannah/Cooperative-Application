...
mycon = sql.connect(host='127.0.0.1', user='root', passwd='', database='OliveLane_cooperative_database')
...
class Co_operative:
    ...
    def register(self):
        ...
        if self.UserAge >= "18":
            ...
            time.sleep(4)
            self.uniqueId = int(numpy.random.random()*10000)
            print(" " + self.uniqueId, "\n This is your unique ID")
            Query = "INSERT INTO chicago_branch (Full_Name, Contact, Username, Address, Occupation, unique_ID) VALUES(%s, %s, %s, %s, %s, %s)"
            Val = (self.UserAge, self.full_Name, self.contact, self.userName, self.Address, self.occupation, self.uniqueId)
    ...
    def savings(self):
        ...
        for self.myBalance in myreg1:
            pass
        self.inputSavings = float(input("How much would you like to save today \nNOTE!!! nothing less than #500 \n>> "))
        if self.inputSavings >= 500.0:
            self.summation = float(self.inputSavings + self.myBalance)
